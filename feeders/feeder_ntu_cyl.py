...
class Feeder(Dataset):
    def __init__(self, data_path, label_path=None, p_interval=1, split='train', random_choose=False, random_shift=False,
                 ...
                 bone=False, vel=False, cyl=False):
        """
        data_path:
        label_path:
    ...
    def load_data(self):
        ...
        elif self.split == 'train' and self.cyl:
            self.data = np.concatenate((npz_data['x_test'][0:3200], npz_data['x_train']), axis=0)
            self.label = np.concatenate((np.where(npz_data['y_test'][0:3200] > 0)[1], np.where(npz_data['y_train'] > 0)[1]), axis=0)
            self.sample_name = ['train_' + str(i) for i in range(len(self.data))]
        else:
