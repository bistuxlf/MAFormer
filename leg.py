...
leg_negative_list = [1, 2, 3, 4, 5, 7, 10, 11, 12, 13, 14, 15, 18, 19, 20, 21,
                     22, 23, 25, 28, 29, 30, 31, 32, 33, 34, 36, 37, 38, 39,
                     40, 41, 44, 45, 46, 47, 49, 50, 53, 54, 56, 57, 58]
leg_positive_list = [6, 8, 9, 16, 17, 24, 26, 27, 35, 42, 43, 48, 51, 52, 55,
                     59, 60]
leg_pos = [19, 18, 17, 16, 0, 12, 13, 14, 15]
...
class Processor():
    ...
    def load_model(self):
        ...
        #    if name == 'fc2' or 'blocks.7' in name:
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch, (data, label, sample) in enumerate(tqdm(self.data_loader['train'], desc="Training", ncols=100)):
            self.global_step += 1
            with torch.no_grad():
                #print("\nleg.py data.shape ==", data.shape)
                data = data[:, :, :, leg_pos, :]
                #print("\nleg.py data2.shape ==", data.shape)
                data = data.float().cuda(self.output_device)
                for idx, i in enumerate(label):
                    label[idx] = 0 if (i+1) in leg_negative_list else 1
            ...
            prec1, prec5 = accuracy(output.data, label, topk=(1, 1))
