...
class Model(nn.Module):
    def __init__(self, len_parts, num_classes, num_joints,
                 ...
                 att_drop=0.0, dropout=0.0, dropout2d=0.0, koopman='t'):
        super().__init__()
    ...
    def Koopman(self, N, V, M, mode, x, K):
        c_new = x.size(1)
        if mode == 't':
        ...
        x1 = x[:, :, :-1]
        x2 = x[:, :, 1:]
        out = (torch.einsum('cij, bjt -> bcit', K, x1) - x2.unsqueeze(1)).norm(dim=-2).mean(dim=-1)
        out = - out
        return out
    ...
    def forward(self, x):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if i == 5:
                out = self.Koopman(N, V, M, 't', x, self.K)
            if i > 5:
                out = 0.8 * self.Koopman(N, V, M, 't', x, self.K) + 0.2 * out
        # NM, C, T, V
        # c_new = x.size(1)
        # if self.koopman == 't':
        #     x = x.view(N, M, c_new, -1, V)  # 64 * 2 * 256 *t*v
...
class Graphformer_Block(nn.Module):
    def __init__(self, in_channels, out_channels, qkv_dim,
                 ...
                 kernel_size, use_pes=True, att_drop=0.0):
        super().__init__()
...
if __name__ == '__main__':
    net = Model(len_parts=6, num_classes=60, num_joints=25,
                num_frames=120, num_heads=3, num_persons=2, num_channels=3,
                kernel_size=[3, 5], use_pes=True, config=[[64, 64, 16], [64, 64, 16],
                                                          [64, 128, 32], [128, 128, 32],
                                                          [128, 256, 64], [256, 256, 64],
                                                          [256, 256, 64], [256, 256, 64]],
                att_drop=0, dropout=0, dropout2d=0, koopman='t').cuda()
    # N, C, T, V, M
    x = torch.randn([1, 3, 120, 25, 2]).cuda()
    y = net(x)
    print(y.shape)
